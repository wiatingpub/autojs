...
var indexFlagText="首页";//首页特有的标志文字
    ...
    if(totalNewsReaded > totalNewsOneTime){
     ...
     while((!isFindNews || lastNewsText === newsText)  && loopTimeToFindNews < 20){
        ...
        newsItem = className("android.support.v4.view.ViewPager")
            .className("android.support.v7.widget.RecyclerView").findOnce(0).child(0).child(0);
        if(newsItem){
            //判断是不是广告,通过是否有阅读数量判断
            if(newsItem.childCount() > 1 && newsItem.child(1) && newsItem.child(1).text() == "广告"){
                newsItem = null;
            }else{
